...
*   **Display & Headlines (IBM Plex Sans Thai):** These are our "Brand Voices." Use `display-md` for daily revenue or total pets. The generous tracking and clear curves of IBM Plex Sans Thai soften the technical nature of the POS while maintaining Thai readability.
*   **Body & Labels (IBM Plex Sans Thai):** These are our "Functional Voices." It provides maximum readability for pet medical notes, appointment times, and SKU numbers. 
...
To create an organic, dynamic background for large containers or column wrappers, utilize "Fluid Mesh Gradients." Place large, heavily blurred geometric shapes (blobs) at opposing corners (e.g., bottom-right and top-left) of a container with `overflow-hidden`. Using very high blur values and theme-specific colors, these overlapping blobs create a soft, ambient glow that gives life to the background without overpowering the content.

### White Frosted Glass (Inner Cards)
When placing interactive cards on top of Fluid Mesh Gradients, they should employ a "White Frosted Glass" effect to stand out while remaining cohesive. Instead of a flat semi-transparent color, use a subtle white gradient (e.g., fading from a higher to a lower opacity) from top-left to bottom-right. Combine this with a strong backdrop blur and a semi-transparent white border. This preserves the brightness of the card so it doesn't "sink" into the background gradient, while still allowing the vibrant colors behind it to elegantly refract and bleed through.
